#pragma once
#include <cstdint>
#include "Protocol.h" // PidAxis, PktSetPid, PktCalibCmd (shared with GCS)

// ---------------------------------------------------------------------------
// SPI frame format — internal protocol between the Flight Controller (master)
// and the ESP32 (slave).
//
// This is completely separate from the GCS UDP protocol (Protocol.h). Its sole
...
struct SpiPayloadStatus {
    ...
    uint8_t wifi_rssi;        // 0–100 (measured on FC side if applicable)
};
...
// ---------------------------------------------------------------------------
// Full fixed-size frame: FC → ESP32 (MOSI)
// The union is sized to SPI_MAX_PAYLOAD so the struct is always SPI_FRAME_SIZE.
// ---------------------------------------------------------------------------
...
struct SpiRxFrame {
    ...
    union {
        ...
        uint8_t               raw[SPI_MAX_PAYLOAD]; // 248 bytes — sizes union
    } payload;                //  248 bytes
...
// ---------------------------------------------------------------------------
// Full fixed-size frame: ESP32 → FC (MISO)
// Carries at most one pending GCS command per SPI transaction.
// has_cmd = 0 means no command — the FC ignores the cmd_* fields.
// ---------------------------------------------------------------------------
struct SpiTxFrame {
    uint16_t magic;    // SPI_MAGIC_ESP_TO_FC
    uint8_t  has_cmd;  // 1 = a valid GCS command is present in cmd
    uint8_t  cmd_type; // PKT_SET_PID (0x10) or PKT_CALIB_CMD (0x20)
    union {
        // Full packet bytes (including header + CRC) so the FC can validate them
        uint8_t set_pid_raw  [sizeof(PktSetPid)];   // 27 bytes
        uint8_t calib_cmd_raw[sizeof(PktCalibCmd)]; // 16 bytes
    } cmd;             //  27 bytes (sized to largest command)
    uint16_t crc;      //   2 bytes — over magic + has_cmd + cmd_type + cmd
